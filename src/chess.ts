...
// Alternative approach - avoid __dirname entirely
...
class Chessboard implements ChessboardProps {
        ...
        private getImagePath(pieceColor: string, pieceType: string): string {
                ...
                const possiblePaths = [
                        // For development
                        path.join(this.projectRoot, 'src', 'public', this.style, filename),
                        path.join(this.projectRoot, 'public', this.style, filename),
                        
                        // For compiled/deployed
                        path.join(this.projectRoot, 'lib', 'public', this.style, filename),
                        path.join(this.projectRoot, 'assets', this.style, filename),
                        
                        // Relative to compiled output
                        path.resolve(this.projectRoot, 'public', this.style, filename),
                        path.resolve(this.projectRoot, 'assets', this.style, filename),
                ...
                throw new Error(`Chess piece image not found: ${filename}. Searched paths: ${possiblePaths.join(', ')}`);
        }
        ...
        private async drawBoard(ctx: SKRSContext2D): Promise<SKRSContext2D> {
                ...
                for (let r = 0; r < 8; r++) {
                        for (let c = 0; c < 8; c++) {
                                ...
                                const coords = (this.cols[colIndex] + rowValue.toString()) as Square;
                                ...
                                if (piece) {
                                        try {
                                                ...
                                                ctx.beginPath();
                                                ctx.rect(x + 5, y + 5, (this.size / 8) - 10, (this.size / 8) - 10);
        ...
        public getSnapshot(move: number): Chess {
                ...
                const pgn = moves.reduce((acc, curr, index) => {
                        const moveNumber = Math.floor(index / 2) + 1;
                        const prefix = index % 2 === 0 ? `${moveNumber}.` : "";
                        return `${acc} ${prefix} ${curr}`.trim();
                }, "");
        ...
        public async buffer(mime: "image/png" | "image/jpeg" | "image/webp" | "image/avif" | "image/gif" = "image/png", options?: BufferOptions): Promise<Buffer> {
                if (mime !== "image/gif") {
                        ...
                        const board = options?.move ? new Chessboard({ 
                                dark: this.dark, 
                        ...
                        if (options?.move) {
                                ...
                                const snapshot = this.getSnapshot(targetMove);
                        ...
                        return new Promise<Buffer>(async (resolve, reject) => {
                                try {
                                        ...
                                        const length = options?.move ? 
                                                Math.min(options.move, moves.length) : 
                                                moves.length;
