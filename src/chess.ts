...
// ES module equivalent of __dirname - using dynamic approach to prevent compiler optimization
const getModuleDir = () => {
...
class Chessboard implements ChessboardProps {
        ...
        private async drawBoard(ctx: SKRSContext2D) {
                ...
                const row = this.flipped ? (r: number) => r + 1 : (r: number) => 7 - r + 1;
                ...
                for (let r = 0; r < 8; r++) {
                        for (let c = 0; c < 8; c++) {
                                ...
                                if (piece) {
                                        const image = await loadImage(path.join(moduleDir, `../public/${this.style}/${piece.color}${piece.type}.png`));
