...
interface CryptoSubtle {
	importKey(format: string, keyData: ArrayBuffer, algorithm: any, extractable: boolean, keyUsages: string[]): Promise<any>
...
declare global {
	var crypto: WebCrypto | undefined
}

// Ensure browser & node compatibility with proper type assertion
function getSubtle(): CryptoSubtle | undefined {
	try {
		// Try browser first
